...
function Book({ href, name, description, children }: { href: string; name: string; description: string; children: React.ReactNode }) {
  return (
    <li className="reveal w-[15rem] shrink-0 snap-start md:w-[17rem] 2xl:w-[19rem]">
      <Link href={href} className="group block rounded-sm focus-visible:outline-offset-4">
        ...
        <p className="mt-5 font-head text-base leading-snug font-bold text-white transition-colors group-hover:text-gold">{name}</p>
...
export async function ShowcaseSection() {
  ...
  return (
    <section aria-labelledby="showcase-heading" className="on-night dark relative isolate overflow-hidden bg-night py-24 text-cream md:py-32">
      ...
      <div className="mx-auto max-w-6xl px-6 2xl:max-w-7xl 2xl:px-8 4xl:max-w-[96rem] 4xl:px-12">
        <div className="grid grid-cols-1 items-end gap-6 md:grid-cols-12">
          <div className="md:col-span-7">
            ...
            <h2 id="showcase-heading" className="display mt-7 text-[2.6rem] text-white md:text-[4rem]" style={{ textWrap: "balance" }}>
              {t("title")}
            </h2>
          </div>
          <p className="font-body text-lg leading-relaxed text-cream/75 italic md:col-span-5">{t("lead")}</p>
        </div>
      </div>

      <ul className="mx-auto mt-16 flex max-w-[100vw] snap-x snap-mandatory gap-8 overflow-x-auto px-6 pb-6 [scrollbar-width:thin] md:gap-10 lg:justify-center lg:overflow-visible lg:flex-wrap 2xl:px-8">
        ...
        {presets.map((p) => (
          ...
          >
            <Image
              ...
              width={544}
              height={720}
              sizes="(min-width: 1536px) 19rem, 17rem"
