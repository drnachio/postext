...
| Section | Settings |
|---------|----------|
| **Page** | Background color, size preset (or custom width/height), margins (top, bottom, left, right), DPI, cut lines, baseline grid (with color and line width), and a **Numbering** subsection (format + start-at) for page labels |
...
| **Headings** | General defaults (font family, line height, color, alignment, weight, margins) plus per-level overrides (H1–H6) for font size, line height, font family, weight, color, margins, a **Break before** toggle with an optional parity selector, a **Span** selector (`column` or `page`), and an **Advanced design** toggle that opens a per-level design slot editor (text, rule, and box elements with anchored placement, overflow modes, hyphenation toggle, and `{titleText}` / `{number}` / `{numberRoman}` placeholders) |
| **Headers & footers** | Per-slot lists of text, rule and box elements. Each text element exposes content (with `{title}`, `{chapterTitle}`, `{pageNumber}`, `{totalPages}` placeholders), font + color controls, parity (all / odd / even pages), an **Overflow** selector (wrap, ellipsis at start/middle/end, clip), a **Hyphenation** toggle, and an anchor + offset + size placement editor for fine positioning |
