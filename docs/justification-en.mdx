...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Increase <code>maxWordSpacing</code> slightly (e.g., from 2 to 2.4). This gives the algorithm more room.</td>
    </tr>
    <tr>
...
In the Sandbox these live in the *Headings* section of **Design → Headings & contents** ("Balance Columns", with "Stretch After Lists", "Stretch Under Figures" and "Loose Paragraphs" nested under it). Turn on the baseline grid (**Design → Advanced → On-screen aids**) and look at the Canvas view to see the effect: with balancing off, short columns end above the last grid line; with it on, every balanceable column closes on the same line. See the [Configuration](/en/docs/configuration#column-balancing) page for the full option reference.
