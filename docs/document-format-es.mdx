...
La directiva `:ref` admite dos atributos opcionales. `style` selecciona cómo se renderiza la etiqueta calculada: `style="number"` imprime el número a secas (`1.7`), `style="full"` imprime el nombre completo del tipo seguido del número (`Figura 1.7`), y cuando `style` se omite se usa la etiqueta corta (`shortLabel`) del tipo seguida del número (`Fig. 1.7`). `text` es una anulación literal que sustituye cualquier etiqueta calculada y tiene prioridad sobre `style`. Los cuatro estilos de renderizado, lado a lado:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Una anulación explícita. El texto dado se usa literalmente en lugar de cualquier etiqueta calculada — útil para enlaces de prosa como "como vimos antes". Cuando está presente, <code>text</code> tiene prioridad sobre <code>style</code>.</td>
    </tr>
  </tbody>
</table>
