...
figures panel with an interactive table editor and image/SVG upload, a fonts panel, and a Design
panel whose eleven groups cover every corner of the engine. Its controls are built on Base UI
primitives (`packages/postext-sandbox/src/ui/`) and checked with axe-core for WCAG 2.1 AA, so new
controls should reuse them and keep every field named by its label and described by its help
text. Every UI string follows a strict i18n
