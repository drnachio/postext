...
5. **Off-main-thread builds.** The `postext/worker` entry point runs the entire pipeline
...
6. **Flat numeric fields.** Bounding boxes are stored as flat `x, y, width, height` fields on
   each node, not as nested objects. This avoids pointer chasing and is more cache-friendly.

7. **Dual-access VDT.** The tree (`pages > columns > blocks`) gives hierarchical access for
