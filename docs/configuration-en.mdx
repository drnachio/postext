...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Placement used by resources of this type that do not set their own <code>placement</code>. When unset, the built-in <code>auto</code> / <code>column</code> default applies. See <a href="#numbering-and-references">Numbering and references</a> below for the full resolution chain.</td>
    </tr>
    <tr>
...
The following configuration sections exist in the type system only — the layout engine does not read them yet. They are included here as a signpost for where the configuration surface is heading.

Two stubs that used to live here have shipped as real features and moved out of this section:

- **Resource placement** is no longer a future enum (`'topOfColumn'`, `'floatLeft'`, `'fullWidthBreak'`, …). The shipped float model uses `position: 'auto' | 'top' | 'bottom' | 'here'` and `span: 'column' | 'page'`, resolved per resource, then the type's `defaultPlacement`, then the built-in `auto` / `column`. See [Resource types](#resource-types) above and [Document format › Resources](/en/docs/document-format#resources).
- **Automatic figure/table numbering** shipped as typed first-reference numbering — see [Resource types](#resource-types).

### Column Config

*Type-system only.* Fine-grained column control: explicit column count, gutter size, column rules (visual separators), and column balancing.

### Typography

*Type-system only.* Advanced typographic controls: orphan/widow minimum line counts, rag optimization, spacing around headings and figures, and keep-together rules for headings with paragraphs and figures with captions.

### References

*Type-system only.* Footnote placement (`'columnBottom'`, `'pageBottom'`, `'endOfSection'`), marker style (`'number'`, `'symbol'`, `'custom'`), and margin notes. (Automatic figure/table numbering, formerly listed here, has shipped — see [Resource types](#resource-types).)

### Section Overrides

*Type-system only.* Apply different column, typography, or resource placement settings to specific sections of the document, matched by CSS-like selectors.

### Renderer

*Type-system only.* Target output format: `'web'` for HTML/canvas rendering, `'pdf'` for print output.
