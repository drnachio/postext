...
Calling `buildDocument` directly runs the full pipeline — parse, measure, seven passes, up to five convergence iterations — on whichever thread invoked it. For a one-shot export that is fine. For an **interactive UI** (live preview, editor, resize-aware viewer) that is the wrong thread: a 150 ms layout blocks input events, keystrokes queue up, and scroll stutters.

Postext ships a dedicated Web Worker entry point — `postext/worker` — that takes the pipeline off the main thread. It is the recommended integration for any UI that rebuilds the document in response to user input. The sandbox's Canvas, HTML and PDF viewports all share the same `createLayoutWorker()` handle through a single `useLayoutWorker` hook (`packages/postext-sandbox/src/worker/useLayoutWorker.ts`) and drive it with last-wins cancellation — a new keystroke aborts the in-flight build before it even finishes.
...
// 1. Register fonts once per family (transferable ArrayBuffers).
...
// 2. Build — supersede a previous call by aborting its signal first.
...
// 3. Dispose when the component that owns the worker unmounts.
...
- **One-shot exports** that run in response to an explicit user action (clicking "Export PDF") and happen to be acceptable at any latency. Calling `buildDocument` directly is simpler and avoids the cost of the initial font transfer.
