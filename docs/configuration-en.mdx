...
interface ElementPlacement {
  ...
  anchor: {
    to: 'container' | `#${string}`; // container = the slot; #id = another element in the slot
    edge: AnchorEdge;
  };
...
- **Container edges** (when `anchor.to === 'container'`): `top`, `top-left`, `top-right`, `bottom`, `bottom-left`, `bottom-right`, `left`, `right`.
