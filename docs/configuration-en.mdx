...
Open the **Custom Fonts** section in the configuration panel. For each family:

1. **Add font family** — creates an empty family; rename it inline.
2. **Upload variant** — pick a weight (100–900) and style (normal / italic), then choose a `.woff2`, `.woff`, `.ttf`, or `.otf` file. The file is stored in IndexedDB and bound to that (weight, style) slot. Re-uploading a slot replaces the previous file.
3. **Delete variant** or **Delete family** — removes the entry from the config *and* the stored bytes from IndexedDB.
...
- The sandbox registers each uploaded variant as a `FontFace` on `document.fonts`, so the HTML viewer and PDF export see it alongside Google-fetched faces.
...
The warnings panel recognises two new failure modes specific to custom fonts (both enabled by the same `debug.warnings.missingFont` toggle that already guards the generic "not loaded" warning):

- **Unknown font family** — a `fontFamily` references a name that is neither a known Google Font nor a custom family. Normally this silently falls back to the system default; the warning surfaces it so you can catch typos or deleted families.
...
Clicking either warning opens the configuration panel so you can rename, re-add, or upload the required variant.
