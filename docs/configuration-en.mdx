...
Each slot holds a list of **text**, **rule**, and **box** elements. Array order is paint order (first element paints first, last element paints on top).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Space above the box (collapses with the previous block's margin) and minimum space below it.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><code>number</code></td>
      <td><code>2</code></td>
      <td>Fewest text lines a fragment of a split box (<code>keepTogether: false</code>) keeps on either side of the cut — a figure or a display formula counts as one line. With the default no box breaks leaving a lone line at the foot of a column or at the head of the next; when no cut satisfies the minimum the box moves whole.</td>
    </tr>
  </tbody>
</table>
