...
- **`'oneAndHalf'`** — An asymmetric layout with a main column and a narrower side column. The side column (controlled by `sideColumnPercent`) is ideal for margin notes, small figures, or supporting content. Values between 25–40% work well. With `sideColumnRole: 'floats'` the body text never enters the side column: it becomes a channel for the figures, tables and callouts placed with `span: 'side'`, each stacked beside the paragraph that first cites it (a resource that cannot fit the rest of the channel waits for the next page's), while `span: 'page'` floats and boxes still cross both columns. Combined with mirrored margins and `sideColumnSide: 'outer'`, the channel sits at the outer edge of every page — the marginal column of a textbook.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>headings font, body size, <code>700</code>, <code>false</code>, main colour, <code>'none'</code>, <code>0.5em</code></td>
      <td>Title typography. <code>gap</code> is the space between the title and the first child (and the icon column gap). <code>textTransform: 'uppercase'</code> is length-preserving.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>List typography inside the box (<code>color</code>, <code>indent</code>, <code>gap</code> and <code>itemSpacing</code> also apply to ordered lists).</td>
    </tr>
    <tr>
      <td><code>marginTop</code> / <code>marginBottom</code></td>
      <td><code>Dimension</code></td>
