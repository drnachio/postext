...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>'column' | 'page'</code></td>
      <td><code>'column'</code></td>
      <td>Horizontal extent: the column, or the full content width. Overridable per instance with the <code>span</code> attribute. Recorded on the VDT; in multi-column layouts <code>'page'</code> is laid out inline at column width for now (pagination phase pending).</td>
    </tr>
    <tr>
      <td><code>placement</code></td>
      <td><code>'here' | 'top' | 'bottom'</code></td>
      <td><code>'here'</code></td>
      <td>Inline in the flow, or floated to a top / bottom page band like a resource. Overridable per instance with the <code>placement</code> attribute. Floating placements are recorded but placed inline for now (pagination phase pending).</td>
    </tr>
    <tr>
      <td><code>width</code></td>
...
- `span: 'page'` in multi-column layouts and `placement: 'top' | 'bottom'` are recorded on the VDT (`VDTBlock.callout`) but still placed inline at column width — full-width and floating boxes arrive with the pagination phase.
