...
- `span: 'page'` in a multi-column layout makes the box a **span block**: it is laid out at the full content width and splits the page into column bands — the text columns above it are closed at the cut line, the box takes its own full-width column, and a fresh band of text columns opens below it, so the flow continues under the box in every column. In this first stage a span block is placed only where the columns are *level*: at the top of a page, right below a `span: 'page'` opener heading, right below another span block, or right below a top float band. Anywhere else (the columns already uneven, or no room for the box plus the widow minimum of body lines below it) the box moves to the top of the next page — without forcing a page break, so the page it leaves is still balanced. Inserting a span block mid-page, with the columns uneven, is the next stage. A heading right before a span block does not travel with it. In single-column layouts `span: 'page'` is simply inline.
