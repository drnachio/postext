...
interface ResourcePlacement {
  ...
  span?: 'column' | 'page';             // single column or full content width
}
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>'column' | 'page'</code></td>
      <td><code>'column'</code></td>
      <td>Horizontal extent: the column, or the full content width. Overridable per instance with the <code>span</code> attribute. In multi-column layouts a <code>'page'</code> box becomes a <em>span block</em>: it splits the page into column bands and sits in its own full-width column (see the container section below).</td>
    </tr>
    <tr>
      <td><code>placement</code></td>
