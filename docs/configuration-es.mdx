...
interface ElementPlacement {
  ...
  anchor: {
    to: 'container' | `#${string}`; // container = el bloque; #id = otro elemento del mismo bloque
    edge: AnchorEdge;
  };
...
- **Bordes del contenedor** (cuando `anchor.to === 'container'`): `top`, `top-left`, `top-right`, `bottom`, `bottom-left`, `bottom-right`, `left`, `right`.
