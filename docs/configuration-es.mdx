...
Abre la sección **Custom Fonts** del panel de configuración. Para cada familia:

1. **Añadir familia** — crea una familia vacía; renómbrala en línea.
2. **Subir variante** — elige un peso (100–900) y un estilo (normal / italic), luego selecciona un archivo `.woff2`, `.woff`, `.ttf` u `.otf`. El archivo se guarda en IndexedDB y queda asociado a esa ranura (peso, estilo). Volver a subir a una ranura existente reemplaza el archivo anterior.
3. **Eliminar variante** o **Eliminar familia** — elimina la entrada de la configuración *y* los bytes guardados en IndexedDB.
...
- El sandbox registra cada variante subida como un `FontFace` en `document.fonts`, de modo que el visor HTML y la exportación a PDF la ven junto a las caras traídas desde Google.
...
El panel de avisos reconoce dos modos nuevos de fallo específicos de las fuentes personalizadas (ambos gobernados por el mismo toggle `debug.warnings.missingFont` que ya controla el aviso genérico "no cargada"):

- **Familia desconocida** — un `fontFamily` referencia un nombre que no es ni una Google Font conocida ni una familia personalizada. Normalmente esto cae en silencio al sistema por defecto; el aviso lo visibiliza para que detectes erratas o familias eliminadas.
...
Al pulsar cualquiera de los dos avisos se abre el panel de configuración para que renombres, vuelvas a añadir o subas la variante necesaria.
