...
- [Estilo de tablas](#estilo-de-tablas) y [Estilo de pies](#estilo-de-pies) — tipografía y decoración de las tablas-recurso y sus pies.
...
El número que produce un tipo de recurso es lo que imprime `:ref` y lo que precede el prefijo del pie. La gramática del lado del documento — la inserción en bloque `::resource{id}` y la referencia en línea `:ref{id}`, junto con sus opciones `style` y `text` — se documenta en [Formato del documento › Recursos](/es/docs/document-format#recursos), incluido cómo el orden de primera referencia determina el conteo.
