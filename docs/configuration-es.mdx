...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Colocación usada por los recursos de este tipo que no definen su propio <code>placement</code>. Si tampoco está definida, se aplica el valor integrado <code>auto</code> / <code>column</code>. Ver <a href="#numeración-y-referencias">Numeración y referencias</a> más abajo para la cadena de resolución completa.</td>
    </tr>
    <tr>
...
- **La colocación de recursos** ya no es un enum futuro (`'topOfColumn'`, `'floatLeft'`, `'fullWidthBreak'`, …). El modelo de flotado implementado usa `position: 'auto' | 'top' | 'bottom' | 'here'` y `span: 'column' | 'page'`, resueltos por recurso, luego por el `defaultPlacement` de su tipo y por último el valor integrado `auto` / `column`. Consulta [Tipos de recurso](#tipos-de-recurso) más arriba y [Formato del documento › Recursos](/es/docs/document-format#recursos).
