...
**Bandas de columnas y columnas a ancho de página.** `page.columns` es un array plano en orden de lectura, pero una página no siempre es una única fila de columnas. Un bloque en línea a ancho de página — hoy un `:::callout` con `span: 'page'` en una disposición multicolumna — corta la página en *bandas* apiladas: las columnas de texto de la banda actual se cierran en la línea de corte (su altura se recorta y `availableHeight` queda a cero), el bloque recibe su propia columna a todo el ancho con `kind: 'span'`, y debajo se añade una banda nueva de columnas de texto (`band + 1`, con la misma x y anchura y el mismo fondo que la banda a la que sustituye). Las columnas solo se añaden al final, así que `columnIndex` sigue apuntando a `page.columns[i]`, y los renderizadores no necesitan ningún trazado especial: cada columna recorta a su propio bbox, y el filete entre columnas se dibuja por banda entre columnas de texto adyacentes. El equilibrado de columnas ignora las columnas a ancho de página y las bandas de altura cero. En esta primera etapa un bloque a ancho de página solo se coloca donde la banda está nivelada (inicio de página, justo después de un encabezado de apertura, de otro bloque a ancho de página o de una banda de flotantes superior); en cualquier otro caso abre la página siguiente.
