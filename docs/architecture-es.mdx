...
- **Título con su primer párrafo.** Un título nunca debe aparecer al final de una columna si el párrafo que introduce empezaría en la columna siguiente. Lo gestiona `headings.keepWithNext` (por defecto `true`): si no hay sitio para el título *más* al menos una línea del bloque siguiente, el título se empuja hacia adelante para viajar con su texto.
