...
- Si elegir este corte dejaría menos de `orphanMinLines` líneas en la parte superior de la siguiente columna, se suman `orphanPenalty` (valor por defecto 3000) a los demerits del nodo.
- Si elegir este corte dejaría menos de `widowMinLines` líneas al final de la columna actual, se suman `widowPenalty` (valor por defecto 3000).
- Si la última línea resultante de este corte fuera más corta que `runtMinCharacters × normalSpaceWidth`, se suman `runtPenalty` (valor por defecto 1500 -- más suave porque los *runts* son estéticos, no estructurales).
