...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Tipografía de celdas, bordes, radio de las esquinas y rellenos de cabecera/cuerpo para recursos <code>kind: 'table'</code>, más las variantes con nombre de <code>tableStyles</code> que una tabla elige con <code>table.styleId</code> — véase <a href="/es/docs/configuration#estilo-de-tablas">Configuración → Estilo de tablas</a></td>
      <td>Pasada 4</td>
    </tr>
    <tr>
...
interface VDTColumn {
  ...
  band?: number;              // banda de columnas (0 salvo que un bloque a ancho de página parta la página)
  kind?: 'text' | 'span';     // 'span' = columna a todo el ancho con un bloque a ancho de página
}
...
**Bandas de columnas y columnas a ancho de página.** `page.columns` es un array plano en orden de lectura, pero una página no siempre es una única fila de columnas. Un bloque en línea a ancho de página — hoy un `:::callout` con `span: 'page'` en una disposición multicolumna — corta la página en *bandas* apiladas: las columnas de texto de la banda actual se cierran en la línea de corte (su altura se recorta y `availableHeight` queda a cero), el bloque recibe su propia columna a todo el ancho con `kind: 'span'`, y debajo se añade una banda nueva de columnas de texto (`band + 1`, con la misma x y anchura y el mismo fondo que la banda a la que sustituye). Las columnas solo se añaden al final, así que `columnIndex` sigue apuntando a `page.columns[i]`, y los renderizadores no necesitan ningún trazado especial: cada columna recorta a su propio bbox (ensanchado por `columnClipRect`: 2pt para la tinta de los glifos más lo que sobresalgan de la columna sus diseños superpuestos, como la pestaña de un título o el distintivo de un recuadro; el mismo rectángulo en los backends canvas y PDF), y el filete entre columnas se dibuja por banda entre columnas de texto adyacentes. El equilibrado de columnas ignora las columnas a ancho de página y las bandas de altura cero. Un bloque a ancho de página corta directamente donde la banda está nivelada (inicio de página, justo después de un encabezado de apertura, de otro bloque a ancho de página o de una banda de flotantes superior); si llega a una banda desigual propone en su lugar un *tope de banda* (`packages/postext/src/pipeline/bandCaps.ts`) — las columnas de la banda que abre un bloque de contenido dado se acortan a `ceil(Σ usado / N / rejilla)` líneas — y `buildDocument` repite la pasada de colocación con ese tope (agrandándolo línea a línea cuando la banda acortada desborda, como mucho unas pocas pasadas adicionales, y pasando después a la página siguiente), de modo que el texto llena las columnas acortadas bajo todas las reglas de colocación, termina nivelado en el corte, y las columnas cerradas conservan la holgura que quede como `availableHeight` para que el equilibrado la absorba. El mismo mecanismo nivela la banda de *cierre* de cada capítulo y del documento (`headings.balancing.trailing`): una apertura de capítulo, un `:::part`, un aviso `placement: 'fixed'` que cierra el capítulo o el final del documento alcanzado con las columnas de la banda actual desiguales propone un tope `kind: 'trailing'`, indexado por el bloque límite; como un tope se indexa por el bloque que abre su banda — y ese bloque se mueve cada vez que una página anterior absorbe líneas de equilibrado —, los topes de cierre se resuelven *después* de que el equilibrado de columnas se haya asentado, con sus ajustes congelados, y una breve ronda de pulido deja que las palancas rellenen lo que el corte dejó corto. Los avisos `placement: 'fixed'` salen del flujo: la caja se ancla al área de contenido / caja de corte / caja de sangre de la página, las columnas de texto que cubre ceden esa zona (recortada por abajo o por arriba como una banda de flotantes, pasando a la página siguiente si hay conflicto), y la caja con sus hijos va a `page.floats`.

### Pasada 4: Colocación de recursos

- **Entrada:** VDT con bloques colocados en columnas
- **Acción:** Flotar cada recurso referenciado al primer hueco libre tras su primera referencia — el final de la columna de la referencia, el principio / el final de la siguiente columna vacía, o una banda de la página siguiente (`packages/postext/src/pipeline/floatPlacement.ts` planifica los flotados, `pipeline/floatSlots.ts` enumera y mide los huecos; el pipeline de construcción reserva las bandas)
- **Resolución de la colocación:** Por recurso, el motor resuelve `resource.placement` → el `resourceType.defaultPlacement` de su tipo → el valor por defecto integrado `{ position: 'auto', span: 'column' }`
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>El recurso toma el primer hueco libre tras su referencia, arriba o abajo — el valor por defecto</td>
    </tr>
    <tr>
      <td><code>position: 'top'</code></td>
      <td>Solo huecos superiores: una banda al principio de la siguiente columna o página vacía, empujando el contenido de la columna por debajo de él</td>
    </tr>
    <tr>
      <td><code>position: 'bottom'</code></td>
      <td>Solo huecos inferiores: una banda al pie de una columna o de la página, acortando la columna por encima de él</td>
    </tr>
    <tr>
...
- **Colocación diferida:** Un flotado que no cabe en ningún hueco de la página actual espera a la siguiente página que abra el flujo — nunca se encoge ni se parte — y, en un límite de capítulo, se vuelca en páginas abiertas por delante del límite
...
- **Regla de post-referencia.** Un flotado aterriza en el primer hueco libre *después* de su primera referencia en el texto, nunca antes. El lector encuentra la referencia primero y luego ve el recurso. Si un flotado no cabe, se difiere hacia adelante a un hueco o página posterior, nunca hacia atrás.
- **Orden de referencia dentro de cada secuencia.** Los flotados pendientes se ofrecen a cada hueco en orden de primera referencia, y uno que no cabe en ninguna parte retiene a los que vienen detrás en su secuencia de numeración: la tabla 3 nunca aterriza después de la tabla 4, la figura 12 nunca antes que la figura 11. Las secuencias no se retienen entre sí — una tabla en espera deja pasar a una figura posterior. Para que una tabla larga no tenga que esperar a una página nueva, la que recibe la cabecera de una columna vacía se corta a esa columna y continúa en el siguiente hueco (la columna de al lado, o las bandas de la página siguiente), con sus filas de cabecera repetidas.
- **Barrera de capítulo.** Los flotados nunca escapan de su capítulo. En una apertura de capítulo (un nivel de encabezado con `breakBefore` o `span: 'page'`), un `:::part`, un estilo de aviso con `floatBarrier: true` y el final del documento, todo flotado pendiente se coloca primero — en los huecos libres de la página, y después en páginas abiertas por delante del límite (cada una fuerza al menos un flotado) — antes del salto de página propio del límite. Antes de abrir esa página, a una figura o tabla aún pendiente se le ofrecen otra vez los huecos libres de la página actual sea cual sea su `position`: un flotado de cabeza de página citado en la página de cierre de un capítulo ocupa el pie de esa página, bajo las columnas equilibradas, en lugar de una página propia (los avisos flotantes conservan su colocación). Un `:::pagebreak` envía los flotados pendientes a la página que le sigue, tras el relleno de paridad.
- **Espacio mínimo de texto.** En una página recién abierta, una banda solo se reserva si todavía caben al menos 3 líneas de cuerpo de texto en las columnas afectadas — con una excepción: un flotado sobredimensionado puede colocarse a la fuerza en una banda que aún es todo texto, para que una figura dominante no pueda atascar la cola indefinidamente. Un hueco de la página actual debe caber en la altura restante de la columna; la regla de las 3 líneas solo se aplica ahí junto a otra banda de flotantes.
...
**El equilibrado converge por segmentos.** Las páginas entre saltos explícitos — la apertura de un capítulo, un `:::pagebreak` — se maquetan con independencia unas de otras: nada fluye a través de un salto así, de modo que una palanca de equilibrado dentro de una tanda de páginas nunca puede mover una línea de otra. El bucle de equilibrado de columnas juzga por eso cada *segmento* por separado. Una pasada sigue colocando el documento entero, pero cada segmento conserva o rechaza su parte de las palancas según su propia puntuación de huecos, veta sus propias cascadas, se estanca por su cuenta y gasta su propio presupuesto de intentos; un segmento cuyo reintento empeoró recupera sus páginas de la mejor pasada que tenía mientras los demás siguen avanzando. Un libro de treinta capítulos se equilibra así exactamente igual que sus capítulos uno a uno — el PDF del libro completo y el PDF del capítulo suelto son idénticos — en lugar de que una cascada en cualquier parte cueste una pasada a todas las páginas del libro.
