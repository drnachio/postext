...
- La **penalización de huérfana** se aplica cuando elegir este corte dejaría menos de `orphanMinLines` líneas en la parte superior de la siguiente columna. El valor por defecto de `orphanPenalty` es `3000`.
- La **penalización de viuda** se aplica cuando el corte dejaría menos de `widowMinLines` líneas al final de la columna actual. El valor por defecto de `widowPenalty` es `3000`.
- La **penalización de *runt*** se aplica cuando la última línea del párrafo sería más corta que aproximadamente `runtMinCharacters × normalSpaceWidth` píxeles. El valor por defecto de `runtPenalty` es `1500` — más suave que las de huérfana/viuda porque los *runts* son estéticos, no estructurales.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>1.5</code></td>
      <td>Límite superior del espaciado entre palabras, como multiplicador del ancho normal del espacio. Con el valor por defecto, los espacios pueden estirarse hasta el 150% de su ancho natural.</td>
    </tr>
    <tr>
      <td><code>minWordSpacing</code></td>
      <td><code>0.8</code></td>
      <td>Límite inferior del espaciado entre palabras, como multiplicador del ancho normal del espacio. Con el valor por defecto, los espacios pueden comprimirse hasta el 80% de su ancho natural.</td>
    </tr>
  </tbody>
</table>
...
Con los valores por defecto (1.5 / 0.8), si el ancho normal del espacio es 4 px:
- Cada espacio puede estirarse 2 px (de 4 px a 6 px)
- Cada espacio puede comprimirse 0.8 px (de 4 px a 3.2 px)
...
Los valores por defecto de 1.5 y 0.8 son un compromiso bien probado — coinciden con el rango tradicionalmente recomendado en la literatura tipográfica y producen buenos resultados en la mayoría de anchos de columna y tamaños de fuente.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Aumenta <code>maxWordSpacing</code> ligeramente (por ej., de 1.5 a 1.8). Esto le da más espacio al algoritmo.</td>
    </tr>
    <tr>
...
const vdt = buildDocument(content, {
  bodyText: {
    ...
    maxWordSpacing: 1.5,   // los espacios se estiran hasta el 150%
    minWordSpacing: 0.8,   // los espacios se comprimen hasta el 80%
  },
