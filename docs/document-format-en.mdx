...
A callout is laid out as one unbreakable box — an optional title, then its content typeset with the style's own body and list typography — and moves whole to the next column or page when it does not fit. Full-width (`span="page"`) and floating (`placement="top"` / `"bottom"`) boxes are recorded but still placed inline in this version. An unknown `:::name` fence is not a container — the line is treated as text, exactly like an unknown directive.
