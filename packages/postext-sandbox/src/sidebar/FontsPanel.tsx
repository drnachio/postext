...
export function FontsPanel() {
  ...
  const customFonts: CustomFontFamily[] = useSandboxSelector(
    (s) => s.config.customFonts ?? [],
  );
  ...
  const addVariantFiles = async (
    ...
    const prepared: CustomFontVariant[] = [];
    ...
    for (const file of files) {
      ...
      prepared.push({ weight, style, fileId, format, fileName: file.name });
    }
    if (rejectedNames.length > 0) {
      setErrors((e) => ({
        ...e,
        [familyName]: `${labels.customFontsVariantUnsupported} (${rejectedNames.join(', ')})`,
      }));
    } else {
      setErrors((e) => {
    ...
    if (!fam) return;
    // Always append — duplicate (weight, style) is allowed; the warnings
    // panel surfaces conflicts so the user can resolve them in the variant
    // settings above.
    updateFamily(familyName, { variants: [...fam.variants, ...prepared] });
  };
  ...
  const hasCustomFonts = customFonts.length > 0;

  const resetAll = async () => {
    await Promise.all(
      customFonts.flatMap((f) =>
        f.variants.map((v) => deleteFontFile(v.fileId).catch(() => undefined)),
      ),
    );
    writeFamilies([]);
  };
              ...
              <div className="px-2 py-2">
                {family.variants.map((variant, index) => (
                  <VariantRow
                    ...
                    onDelete={() => {
                      void removeVariant(family.name, index);
                    }}
                  />
                ))}
                ...
                <UploadVariantButton
                  labels={labels}
                  onPick={(files, weight, style) => {
                    void addVariantFiles(family.name, files, weight, style);
                  }}
                />
                {errors[family.name] && (
  ...
  return (
    <div className="mb-1.5 flex items-center gap-1">
      ...
      <span
        ...
        style={{ color: 'var(--slate)', fontSize: 12, lineHeight: '16px' }}
        title={variant.fileName ?? `.${variant.format}`}
      >
...
function UploadVariantButton({
  ...
  labels: SandboxLabels;
  onPick: (files: File[], weight: number, style: CustomFontStyle) => void;
}) {
    ...
    <div className="mt-1 flex items-center gap-1">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (files.length > 0) onPick(files, pendingWeight, pendingStyle);
          e.target.value = '';
        }}
        className="hidden"
        aria-hidden="true"
