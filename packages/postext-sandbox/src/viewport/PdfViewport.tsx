...
import { perfMark, perfSpan } from '../perf/marks';
...
export function PdfViewport() {
  ...
  // The PDF is written on a worker of this tab's own (the layout worker is
  // shared and busy with previews); fonts and SVG rasters are answered here.
  const pdfWorkerRef = useRef<PdfWorkerHandle | null>(null);
  const pdfWorker = (): PdfWorkerHandle => {
    if (!pdfWorkerRef.current) pdfWorkerRef.current = createPdfWorker();
    return pdfWorkerRef.current;
  };
  useEffect(() => {
    return () => {
      pdfWorkerRef.current?.dispose();
      pdfWorkerRef.current = null;
    };
  }, []);
  const [renderProgress, setRenderProgress] = useState<RenderProgress | null>(null);
  const renderPhaseRef = useRef<RenderProgress['phase'] | null>(null);
  const currentUrlRef = useRef<string | null>(null);
  const hasAutoGeneratedRef = useRef(false);
  /** Whether the last generation ended in an error (see the font hook). */
  ...
  const regenerate = useCallback(async () => {
    ...
    try {
      ...
      if (scope === 'book') {
        ...
        const doc = await layoutWorker.build(
          ...
          snapshotConfig,
          {
            ...
            // A chapter's document is the one the canvas shows: worth keeping.
      ...
      layoutSpan.end({ pages: pageCount, docs: docs.length });
      setPhase('render');
      ...
      const bytes = await pdfWorker().render(docs, {
        ...
        onProgress: (p) => {
          ...
          if (p.phase !== renderPhaseRef.current) {
            renderPhaseRef.current = p.phase;
            perfMark('pdf.render.phase', { phase: p.phase, pages: p.pages, totalPages: p.totalPages });
          }
        },
      });
  ...
  useEffect(() => {
    ...
    // The document on screen is the other scope's: take it down before the
    // progress shows, rather than report the new one over it.
    if (currentUrlRef.current) URL.revokeObjectURL(currentUrlRef.current);
  ...
  // painted from stale bytes; flag it like any other input change. A
  // generation that ran before the families were registered failed for want
  // of them — nothing is on screen to flag, so run it again now that they
  // are there.
  ...
  useEffect(() => {
    return onCustomFontsChanged(() => {
      ...
      else if (failedRef.current) void regenerateRef.current();
    });
  }, []);
